...
export interface ClientOptions {
  /**
   * TokenRouter API key for authentication. Get your API key from the TokenRouter dashboard.
   *
   * Format: `Authorization: Bearer tr_your_api_key_here`
   *
   */
   ...
   *
   * Defaults to process.env['TOKENROUTER_BASE_URL'].
   */
   ...
   * Set the log level.
   *
   * Defaults to process.env['TOKENROUTER_LOG'] or 'warn' if it isn't set.
   */
...
/**
 * API Client for interfacing with the Tokenrouter API.
 */
export class Tokenrouter {
   ...
   * API Client for interfacing with the Tokenrouter API.
   *
   * @param {string | undefined} [opts.apiKey=process.env['TOKENROUTER_API_KEY'] ?? undefined]
   * @param {Environment} [opts.environment=production] - Specifies the environment URL to use for the API.
   * @param {string} [opts.baseURL=process.env['TOKENROUTER_BASE_URL'] ?? https://api.tokenrouter.io] - Override the default base URL for the API.
   ...
   */
  constructor({
    baseURL = readEnv('TOKENROUTER_BASE_URL'),
    apiKey = readEnv('TOKENROUTER_API_KEY'),
    ...opts
  }: ClientOptions = {}) {
    if (apiKey === undefined) {
      throw new Errors.TokenrouterError(
        "The TOKENROUTER_API_KEY environment variable is missing or empty; either provide it, or instantiate the Tokenrouter client with an apiKey option, like new Tokenrouter({ apiKey: 'My API Key' }).",
      );
    }
    ...
    this.timeout = options.timeout ?? Tokenrouter.DEFAULT_TIMEOUT /* 1 minute */;
    ...
    this.logLevel =
      ...
      parseLogLevel(readEnv('TOKENROUTER_LOG'), "process.env['TOKENROUTER_LOG']", this) ??
      defaultLogLevel;
    ...
    const customHeadersEnv = readEnv('TOKENROUTER_CUSTOM_HEADERS');
    if (customHeadersEnv) {
  ...
  static Tokenrouter = this;
  static DEFAULT_TIMEOUT = 60000; // 1 minute

  static TokenrouterError = Errors.TokenrouterError;
